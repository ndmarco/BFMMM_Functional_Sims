...
### Set working dir
setwd("/Volumes/External_Nick/BFMMM_Sim1_3")
...
rescale_function <- function(Z){
  ...
  min(X[,1])
  max(X[,1])
  trans_mat <- rbind(X[which.max(X[,1]),], X[which.max(X[,2]),])
  trans_mat <- solve(trans_mat)
  X_tilde <- X %*% trans_mat
  Z_new <- Z
  Z_new[,1] <- X_tilde[,1]
  Z_new[,2] <- X_tilde[,2] * (Z[,2]/ (Z[,2] + Z[,3]))
  Z_new[,3] <- X_tilde[,2] * (Z[,3]/ (Z[,2] + Z[,3]))

  trans_mat <- solve(t(Z) %*% Z) %*% t(Z) %*% Z_new


  new_Z <- Z %*% trans_mat
  new_Z_scaled <- new_Z
  i = 1
  while(min(new_Z_scaled) < 0){
    dif <- i * 0.001
    t_mat <- matrix(c(1 - dif, 0.5 * dif, 0.5 * dif, 0.5*  dif, 1- dif, 0.5 * dif, 0.5 * dif, 0.5 * dif, 1-dif), ncol = 3,
                    nrow=3, byrow = T)
    new_Z_scaled <- new_Z %*% t_mat
    i = i + 1
  }

  new_Z_2 <- Z %*% trans_mat %*% t_mat

  cbind(new_Z_2, new_Z_scaled)

  X <- new_Z_scaled[,2:3]
  X[,1] <- new_Z_scaled[,2] / (new_Z_scaled[,2] + new_Z_scaled[,3])
  X[,2] <- new_Z_scaled[,3] / (new_Z_scaled[,2] + new_Z_scaled[,3])


  trans_mat2 <- rbind(X[which.max(X[,1]),], X[which.max(X[,2]),])
  trans_mat2 <- solve(trans_mat2)
  Z_new <- new_Z_scaled
  Z_new[,2:3] <- (X %*% trans_mat2) * (new_Z_scaled[,2] + new_Z_scaled[,3])

  trans_mat2 <- solve(t(new_Z_scaled) %*% new_Z_scaled) %*% t(new_Z_scaled) %*% Z_new

  new_Z2 <- new_Z_scaled %*% trans_mat2
  new_Z_scaled2 <- new_Z2
  i = 1
  t_mat2 <- diag(1,3)
  while(min(new_Z_scaled2) < 0){
    dif <- i * 0.001
    t_mat2 <- matrix(c(1 - dif, 0.5 * dif, 0.5 * dif, 0.5*  dif, 1- dif, 0.5 * dif, 0.5 * dif, 0.5 * dif, 1-dif), ncol = 3,
                     nrow=3, byrow = T)
    new_Z_scaled2 <- new_Z2 %*% t_mat2
    i = i + 1
  }

  new_Z_3 <- new_Z_scaled %*% trans_mat2 %*% t_mat2

  new_Z3 <- new_Z_3 %*% trans_mat2
  new_Z_scaled3 <- new_Z3
  i = 1
  t_mat3 <- diag(1,3)
  while(min(new_Z_scaled3) < 0){
    dif <- i * 0.001
    t_mat3 <- matrix(c(1 - dif, 0.5 * dif, 0.5 * dif, 0.5*  dif, 1- dif, 0.5 * dif, 0.5 * dif, 0.5 * dif, 1-dif), ncol = 3,
                     nrow=3, byrow = T)
    new_Z_scaled3 <- new_Z3 %*% t_mat3
    i = i + 1
  }
...
for(j in 1:3){
  ...
  for(i in 1:10){
    ...
    for(i in 1:4500){
      Z_rescaled[,,i] <- rescale_function(Z_est$Z_trace[,,i])
      if(i %% 100 == 0){
        print(i)
      }
    }
    ...
    for(i in 1:nrow(Z_rescaled)){
      for(j in 1:ncol(Z_rescaled)){
        Z_median[i,j] = median(Z_rescaled[i,j,])
      }
    }
